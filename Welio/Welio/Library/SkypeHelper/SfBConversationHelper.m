...
- (instancetype)initWithConversation:(SfBConversation *)conversation
    ...
    if (self = [super init]) {
        ...
        _audioService = _conversation.audioService;
        [self subscribeTo:_audioService selector:@selector(muted)];
        ...
        [self subscribeTo:_videoService selector:@selector(canSetPaused)];
...
- (void)remove {
    ...
    self.otherVideo = nil;
    [self unsubscribeFrom:_videoService selector:@selector(canSetPaused)];
    ...
    [self unsubscribeFrom:_audioService selector:@selector(muted)];
...
- (void)videoTimer {
    ...
    if ((_videoStream = [_otherVideo subscribe:_incomingVideoLayer error:&error])) {
        ...
        NSLog(@"subscribe subscribe subscribe");
        [self stopTimer];
        return;
    }
    NSLog(@"not subscribe subscribe subscribe");
    _videoStream = nil;
    _displayLink.paused = YES;
}
...
- (void)setVideoEnabled:(BOOL)enabled {
    if (enabled) {
        ...
        if ((_videoStream = [_otherVideo subscribe:_incomingVideoLayer error:&error])) {
            _displayLink.paused = NO;
            NSLog(@"subscribe subscribe subscribe - 1");
            [self stopTimer];
            return;
        }
    ...
    NSLog(@"not subscribe subscribe subscribe - 1");
    _videoStream = nil;
    _displayLink.paused = YES;
}
    ...
    if (object == _conversation) {
        ...
        if ([keyPath isEqualToSelector:@selector(dominantSpeaker)]) {
            ...
            if (newValue && ![newValue isEqual:_conversation.selfParticipant]) {
                SfBPerson *persion = [newValue person];
                NSLog(@"------------%@",persion.displayName);
                if (![persion.displayName isEqualToString:_userInfo[@"displayName"]]) {
                    self.otherVideo = [newValue video];
                    _otherVideoCheck = [newValue video];
                    NSLog(@"set other video");
                }else{
                    NSLog(@"not set other video");
                }
            }
            return;
        }
...
//                SfBPerson *persion = [[newValue firstObject] person];
//                if (![persion.displayName isEqualToString:_userInfo[@"displayName"]]) {
//                    self.otherVideo = [[newValue firstObject] video];
//                }
                NSLog(@"set other video - 2");
                if (_conversation.remoteParticipants.count>0) {
                    for (SfBParticipant *part in _conversation.remoteParticipants) {
                        if (![part.person.displayName isEqualToString:_userInfo[@"displayName"]]) {
                            self.otherVideo = part.video;
                            _otherVideoCheck = part.video;
                            break;
                        }
                    }
                } else {
...
//                }
                ...
                if (_conversation.remoteParticipants.count>0) {
                    ...
                    if (!check) {
                        if (!checkRemoteParticipant) {
                            checkRemoteParticipant = true;
                            if ([delegate respondsToSelector:@selector(conversationHelper:conversationLeave:)]) {
                                [delegate conversationHelper:self conversationLeave:_conversation];
                            }
                        }
    ...
    } else if (object == _audioService) {

        if ([keyPath isEqualToSelector:@selector(muted)]) {
            if ([delegate respondsToSelector:@selector(conversationHelper:audioService:didChangeMuted:)]) {
                [delegate conversationHelper:self audioService:_audioService didChangeMuted:[newValue intValue]];
            }
            return;
        }
        ...
        if ([keyPath isEqualToSelector:@selector(canSetPaused)]) {
            if ([delegate respondsToSelector:@selector(conversationHelper:videoService:didChangeCanSetPaused:)]) {
                [delegate conversationHelper:self videoService:_videoService didChangeCanSetPaused:[newValue boolValue]];
            }
            return;
        }
...
- (void)subcribe:(CAEAGLLayer *)incomingVideoLayer {
    NSError *error = nil;
    _videoStream = nil;
    _displayLink.paused = YES;
    _incomingVideoLayer = incomingVideoLayer;
